 ...
 * Dynamically adapts colors between Light and Dark themes
 */
export function initAuroraBg(canvasId = 'auroraCanvas') {
  ...
  // Render smooth wave mesh adapted to light/dark themes
  function render() {
    time += 0.003;
    ...
    const rad1 = Math.max(width, height) * 0.48;
    ...
    if (isDark) {
      ...
      grad1.addColorStop(0.5, 'rgba(186, 230, 253, 0.06)');
    ...
    const rad2 = Math.max(width, height) * 0.52;
    ...
    if (isDark) {
      ...
      grad2.addColorStop(0, 'rgba(129, 140, 248, 0.11)');
      grad2.addColorStop(0.5, 'rgba(224, 231, 255, 0.05)');
      grad2.addColorStop(1, 'rgba(248, 250, 252, 0)');
    }
    ctx.fillStyle = grad2;
    ctx.fillRect(0, 0, width, height);

    // Aurora blob 3 - Emerald/Teal hint
    const x3 = width * 0.5 + Math.sin(time * 0.5) * (width * 0.25);
    const y3 = height * 0.65 + Math.cos(time * 0.7) * (height * 0.15);
    const rad3 = Math.max(width, height) * 0.42;
    ...
    if (isDark) {
      ...
      grad3.addColorStop(0, 'rgba(52, 211, 153, 0.08)');
      grad3.addColorStop(0.5, 'rgba(209, 250, 229, 0.04)');
